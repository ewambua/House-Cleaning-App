  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/register', {
        ...
        body: JSON.stringify({ user: { name, email, password, password_confirmation: passwordConfirmation, role } }),
      });
      ...
      if (response.ok) {
        ...
        // Handle signup error
        console.log('Signup failed. Error:', errorData.error);
      }
    } catch (error) {
    ...
    <div className='signup'>
      <form onSubmit={handleSubmit} className="form_main">
        ...
        <div className="inputContainer">
          ...
          <input type="text" className="inputField" id="name" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} />
        </div>
        ...
        <svg className="inputIcon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#2e2e2e" viewBox="0 0 16 16">
          ...
          <input type="email" className="inputField" id="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
        </div>
